...
from discord import voice_client       # dumb random number generator
...
async def play(ctx):
    if (ctx.author.voice):
            ...
            await ctx.send("m-am conectat la voice channel")

            source = FFmpegPCMAudio('borat.mp3')
            player = voice.play(source)
    else:
        await ctx.send("nu esti intr-un voice channel")

@bot.command(pass_context = True)
async def leave(ctx):
    if(ctx.voice_client):
        await ctx.guild.voice_client.disconnect()
        await ctx.send("am parasit voice channel-ul")
    else:
        await ctx.send("nu sunt intr-un voice channel")
